...
class DFA(object):
	...
	def draw(self):
		...
		#print(A)
		#print Source(nx.drawing.nx_agraph.to_agraph(g))
		return A
		...
		for s in d.get():
			for item in s:
				if item in self.final_states:
					new_final_states.append((d.find_set(item)))
					break

		self.transitions = {((d.find_set(k[0])),k[1]):(d.find_set(v))  
								for k,v in self.transitions.items()}
		automata_transitions = self.transitions
		self.final_states = new_final_states
		final_states = self.final_states
		start_state=self.start_state
		num_of_state=len(self.states)
		return automata_transitions, final_states,start_state,num_of_state
	...
	#	return num_of_state,num_of_final
